...
/**
 * Handles the {@code QueryByIdsed} annotation for javac.
 */
@ProviderFor(JavacAnnotationHandler.class)
public class HandleQueryByIdsed extends JavacAnnotationHandler<QueryByIdsed> {

	private static final String Before_Name = "queryByIdsedBefore";
	private static final String After_Name = "queryByIdsedAfter";
	private static final String Module_Discript = "id 集合，包括删除";
	private static final String Method_Arg_Name = "ids";
	private static final String Module_Key = "queryByIdsed";

	@Override
	public void handle(AnnotationValues<QueryByIdsed> annotation, JCAnnotation ast, JavacNode annotationNode) {
		// annotationNode 是上下文
		// 判断是否已经使用了注解
		// handleFlagUsage(annotationNode, ConfigurationKeys.TO_STRING_FLAG_USAGE, "@QueryByIdsed");

		// 如果存在则删除
		deleteAnnotationIfNeccessary(annotationNode, QueryByIdsed.class);
		...
		// 获取该注解，并接下来获取该注解上的属性
		QueryByIdsed ann = annotation.getInstance();
		java.util.List<Object> extensionProviders = annotation.getActualExpressions("value");
		...
		generateQueryByIdsed(typeNode, annotationNode, beanClassName, true);
	}

	public void generateQueryByIdsed(JavacNode typeNode, JavacNode source, String beanClass, boolean whineIfExists) {
		...
		if (notAClass) {
			source.addError("@QueryByIdsed is only supported on a class or enum.");
			return;
		}
		...
		switch (methodExists(Before_Name, typeNode, 1)) {
			case NOT_EXISTS:
				JCMethodDecl method = createQueryByIdsedBefore(typeNode, source.get());
			...
			case EXISTS_BY_USER:
				isExistsBefore = true;
				if (whineIfExists) {
					source.addWarning("Not generating queryByIdsed(): A method with that name already exists");
				}
				break;
		}
		...
		switch (methodExists(After_Name, typeNode, 1)) {
			case NOT_EXISTS:
				JCMethodDecl method = createQueryByIdsedAfter(typeNode, beanClass, source.get());
				injectMethod(typeNode, method);
				isExistsAfter = true;
				break;
			case EXISTS_BY_LOMBOK:
		...
		switch (methodExists(Module_Key, typeNode, 0)) {
			case NOT_EXISTS:
				JCMethodDecl method = createQueryByIdsed(typeNode, beanClass, source.get(), isExistsBefore, isExistsAfter);
				injectMethod(typeNode, method);
				break;
			case EXISTS_BY_LOMBOK:
				break;
	...
	static JCMethodDecl createQueryByIdsedBefore(JavacNode typeNode, JCTree source) {
		...
		Name queryByIdsedBeforeName = typeNode.toName(Before_Name);
		// protected
		...
		// protected void queryByIdsedBefore(Long id) {}
		JCMethodDecl queryByIdsedBeforeMethod = maker.MethodDef(modifiers, queryByIdsedBeforeName, beforeType,
				List.<JCTypeParameter>nil(), List.of(LongId), List.<JCExpression>nil(), block, null);
		return recursiveSetGeneratedBy(queryByIdsedBeforeMethod, source, typeNode.getContext());
	}

	static JCMethodDecl createQueryByIdsedAfter(JavacNode typeNode, String beanClassName, JCTree source) {
		JavacTreeMaker maker = typeNode.getTreeMaker();
		Name longIdName = typeNode.toName(Method_Arg_Name);
		JCExpression keyType = chainDotsString(typeNode, "java.lang.String");
		...
		Name queryByIdsedAfterName = typeNode.toName(After_Name);
		// protected
		JCModifiers modifiers = maker.Modifiers(Flags.PROTECTED);
		// void
		JCExpression beforeType = maker.Type(createVoidType(typeNode.getSymbolTable(), CTC_VOID));
		// {}
		JCBlock block = maker.Block(0, List.<JCStatement>nil());
		// protected void queryByIdsedAfter(Long id, List<SysOrg> list) {}
		JCMethodDecl queryByIdsedAfterMethod = maker.MethodDef(modifiers, queryByIdsedAfterName, beforeType,
				List.<JCTypeParameter>nil(), List.of(LongId, listLeft), List.<JCExpression>nil(), block, null);
		return recursiveSetGeneratedBy(queryByIdsedAfterMethod, source, typeNode.getContext());
	}

	static JCMethodDecl createQueryByIdsed(JavacNode typeNode, String beanClassName, JCTree source, boolean isExistsBefore, boolean isExistsAfter) {
		JavacTreeMaker maker = typeNode.getTreeMaker();
		// 添加一个 注解
		...
		// 添加一个 注解 @org.springframework.validation.annotation.Validated({com.zyf.valid.QueryByIdsed.class})
		...
		// Ebean.find(SysOrg.class).setIncludeSoftDeletes()
		JCExpression setIncludeSoftDeletesEnd = maker.Apply(List.<JCExpression>nil(), setIncludeSoftDeletes, List.<JCExpression>nil());

		// Ebean.find(SysOrg.class).setIncludeSoftDeletes().where
		JCFieldAccess where = maker.Select(setIncludeSoftDeletesEnd, typeNode.toName("where"));
		// Ebean.find(SysOrg.class).setIncludeSoftDeletes().where()
		...
		// 判断 queryByIdsedBefore 方法是否存在
		JCStatement beforeStatement = null;
		if (isExistsBefore) {
			// queryByIdsedAfter
			// queryByIdsedBefore
			JCExpression beforeMethod = maker.Ident(queryByIdsedBeforeName);
			// id
			JCExpression beforeArgs = maker.Ident(longIdName);
			// queryByIdsedBefore(id)
		...
		// method
		JCMethodDecl queryByIdsed = maker.MethodDef(mods, typeNode.toName(Module_Key), returnType,
				List.<JCTypeParameter>nil(), List.of(LongId), List.<JCExpression>nil(), body, null);
		return recursiveSetGeneratedBy(queryByIdsed, source, typeNode.getContext());
	}
}
