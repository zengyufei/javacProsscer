...
public class HandleRemoveById extends JavacAnnotationHandler<RemoveById> {
    ...
    private static final String Before_Method_Name = "removeByIdBefore";
    private static final String After_Method_Name = "removeByIdAfter";
    private static final String Module_Discript = "移除";
    private static final String Method_Arg_Name = "id";
    private static final String Method_Name = "removeById";
    ...
    public void handle(AnnotationValues<RemoveById> annotation, JCAnnotation ast, JavacNode annotationNode) {
        ...
        try {
            generateRemoveById(typeNode, annotationNode, beanClassName, isRollback,false);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void generateRemoveById(JavacNode typeNode, JavacNode source, String beanClassName, boolean isRollback, boolean whineIfExists) {
        ...
        switch (methodExists(Before_Method_Name, typeNode, 1)) {
            case NOT_EXISTS:
                JCMethodDecl method = createRemoveByIdBefore(typeNode, beanClassName, source.get());
        ...
        switch (methodExists(After_Method_Name, typeNode, 1)) {
            case NOT_EXISTS:
                JCMethodDecl method = createRemoveByIdAfter(typeNode, beanClassName, source.get());
                injectMethod(typeNode, method);
                isExistsAfter = true;
                break;
            case EXISTS_BY_LOMBOK:
        ...
        switch (methodExists(Method_Name, typeNode, 1)) {
            case NOT_EXISTS:
                JCMethodDecl method = createRemoveById(typeNode, beanClassName, isRollback, source.get());
                injectMethod(typeNode, method);
                break;
            case EXISTS_BY_LOMBOK:
                break;
    ...
    static JCMethodDecl createRemoveById(JavacNode typeNode, String beanClassName, boolean isRollback, JCTree source) {
        JavacTreeMaker maker = typeNode.getTreeMaker();
        ListBuffer bodyBlockList = new ListBuffer();
        JCBlock body;
        Name removeByIdBeforeName = typeNode.toName(Before_Method_Name);
        Name removeByIdAfterName = typeNode.toName(After_Method_Name);

        /*
            private boolean removeByIdBefore(T id) {}
            ...
            public Msg remoteById(@NotNull(message = "Id 不能为空")
                                  ...
                                  Long id) {
                String msg = removeByIdBefore(id);
        ...
        * */
        ...
        // removeByIdBefore
        JCExpression beforeMethodLeft = maker.Ident(removeByIdBeforeName);
        // obj
        ...
        // String msg = removeByIdBefore(obj)
        ...
        // removeByIdAfter
        JCExpression afterMethodLeft = maker.Ident(removeByIdAfterName);
        // obj
        ...
        // method
        JCMethodDecl removeById = maker.MethodDef(methodAccess, typeNode.toName(Method_Name), methodReturnType,
    ...
    static JCMethodDecl createRemoveByIdBefore(JavacNode typeNode, String beanClassName, JCTree source) {
        JavacTreeMaker maker = typeNode.getTreeMaker();
        Name removeByIdBeforeName = typeNode.toName(Before_Method_Name);
        Name objName = typeNode.toName(Method_Arg_Name);
        // Long
        ...
        // private String removeByIdBefore(Long id) { return ""; }
        ...
        return recursiveSetGeneratedBy(removeByIdBeforeMethod, source, typeNode.getContext());
    }

    static JCMethodDecl createRemoveByIdAfter(JavacNode typeNode, String beanClassName, JCTree source) {
        JavacTreeMaker maker = typeNode.getTreeMaker();
        Name removeByIdAfterName = typeNode.toName(After_Method_Name);
        Name objName = typeNode.toName(Method_Arg_Name);
        ...
        // protected void removeByIdAfter(Long id) {}
        JCMethodDecl removeByIdAfterMethod = maker.MethodDef(modifiers, removeByIdAfterName, afterType,
                List.<JCTypeParameter>nil(), List.of(obj), List.<JCExpression>nil(), block, null);
        return recursiveSetGeneratedBy(removeByIdAfterMethod, source, typeNode.getContext());
    }
