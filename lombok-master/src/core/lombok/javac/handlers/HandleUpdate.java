...
public class HandleUpdate extends JavacAnnotationHandler<Update> {
	...
	private static final String Before_Method_Name = "updateBefore";
	private static final String After_Method_Name = "updateAfter";
	...
	public void handle(AnnotationValues<Update> annotation, JCAnnotation ast, JavacNode annotationNode) {
		...
		try {
			generateUpdate(typeNode, annotationNode, beanClassName, dtoClassName, isRollback, false);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	public void generateUpdate(JavacNode typeNode, JavacNode source, String beanClassName, String dtoClassName, boolean isRollback, boolean whineIfExists) {
		...
		switch (methodExists(Method_Name, typeNode, 1)) {
			case NOT_EXISTS:
				JCMethodDecl method = createUpdate(typeNode, beanClassName, dtoClassName, isExistsBefore, isExistsAfter, isRollback, source.get());
				injectMethod(typeNode, method);
				break;
			case EXISTS_BY_LOMBOK:
				break;
	...
	static JCMethodDecl createUpdateBefore(JavacNode typeNode, String beanClassName, JCTree source) {
		...
		Name updateBeforeName = typeNode.toName(Before_Method_Name);
	...
	static JCMethodDecl createUpdateAfter(JavacNode typeNode, String beanClassName, JCTree source) {
		JavacTreeMaker maker = typeNode.getTreeMaker();
		Name updateAfterName = typeNode.toName(After_Method_Name);
		Name objName = typeNode.toName(Method_Arg_Name);
	...
	static JCMethodDecl createUpdate(JavacNode typeNode, String beanClassName, String dtoClassName, boolean isExistsBefore, boolean isExistsAfter, boolean isRollback, JCTree source) {
		JavacTreeMaker maker = typeNode.getTreeMaker();
        ...
        * */
		...
		// method
		JCMethodDecl update = maker.MethodDef(methodAccess, typeNode.toName(Method_Name), methodReturnType,
