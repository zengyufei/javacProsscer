...
public class HandleInsert extends JavacAnnotationHandler<Insert> {
	...
	public void handle(AnnotationValues<Insert> annotation, JCAnnotation ast, JavacNode annotationNode) {
		...
		generateInsert(typeNode, annotationNode, beanClassName, dtoClassName, true);
	}

	public void generateInsert(JavacNode typeNode, JavacNode source, String beanClassName, String dtoClassName, boolean whineIfExists) {
		...
		switch (methodExists(Module_Key, typeNode, 1)) {
			case NOT_EXISTS:
				JCMethodDecl method = createInsert(typeNode, beanClassName, dtoClassName, isExistsBefore, isExistsAfter, source.get());
				injectMethod(typeNode, method);
				break;
			case EXISTS_BY_LOMBOK:
				break;
	...
	static JCMethodDecl createInsert(JavacNode typeNode, String beanClassName, String dtoClassName, boolean isExistsBefore, boolean isExistsAfter, JCTree source) {
		JavacTreeMaker maker = typeNode.getTreeMaker();
        ...
        * */
		...
		// method
		JCMethodDecl insert = maker.MethodDef(methodAccess, typeNode.toName(Module_Key), methodReturnType,
