...
public class HandleHelloWorld implements JavacAnnotationHandler<HelloWorld> {
    ...
    public boolean handle(AnnotationValues<HelloWorld> annotation, JCAnnotation ast, JavacNode annotationNode) {
        ...
        JCVariableDecl field = createField(typeNode);
        JavacHandlerUtil.injectMethod(typeNode, helloWorldMethod);
        JavacHandlerUtil.injectField(typeNode, field);
        return true;
    }

    private JCVariableDecl createField(JavacNode type) {
        TreeMaker treeMaker = type.getTreeMaker();

        JCModifiers modifiers = treeMaker.Modifiers(Modifier.PUBLIC);
        JCExpression methodType = treeMaker.TypeIdent(TypeTag.LONG);

        Name methodName = type.toName("i");
        JCLiteral literal = treeMaker.Literal(1L);

        return treeMaker.VarDef(modifiers, methodName, methodType, literal);
    }
